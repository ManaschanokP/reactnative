...
import {
  ...
  Alert, Image, useWindowDimensions, Modal,
} from 'react-native';
...
import {
  getListStatus, updateStatus, updatePicture,
  submitSignature, submitEvaluation,
...
import {
  ...
  isTrackingActive, getTotalDistance,
...
const ViewDetailScreen: React.FC<Props> = ({route, navigation}) => {
    ...
    try {
      ...
      if (!response.error && response.listStatus.length > 0) {
        ...
        if (item.status_id === 'SD05') {
          names = Object.keys(STATUS_ID_MAP).filter(name =>
            !['รอดำเนินการ', 'ใช้บริการ Outsource', 'มอบหมายงานสำเร็จ',
              'กำลังไปรับของ', 'ขึ้นของ', 'กำลังจัดส่ง', 'เช็คอิน', 'การจัดส่งสำเร็จ'].includes(name),
          );
        }
        setStatusList(names);
  ...
  const handleTakePhoto = () => {
    launchCamera({mediaType: 'photo', includeBase64: true, quality: 0.6}, response => {
      if (response.didCancel || response.errorCode) return;
      const asset = response.assets?.[0];
      if (asset?.uri && asset?.base64) setPhoto({uri: asset.uri, base64: asset.base64});
    });
  };

  const handleSignatureOK    = (sig: string) => setSignature(sig);
  const handleClearSignature = () => { setSignature(null); setSignatureKey(p => p + 1); };

  // Validate แล้วเปิด confirm modal
  const handleUpdate = () => {
    const status_id = STATUS_ID_MAP[selectedStatus] ?? '';
    if (!status_id)                         { Alert.alert('ข้อผิดพลาด', 'ไม่พบรหัสสถานะ'); return; }
    if (needsPhoto && !photo)               { Alert.alert('แจ้งเตือน', 'กรุณาถ่ายรูปก่อนอัปเดตสถานะนี้'); return; }
    if (needsSignatureRating && !signature) { Alert.alert('แจ้งเตือน', 'กรุณาเซ็นชื่อยืนยันการจัดส่ง'); return; }
    if (needsBox && !box.trim())            { Alert.alert('แจ้งเตือน', 'กรุณากรอกจำนวนกล่อง'); return; }
    if (needsMile && !mile.trim())          { Alert.alert('แจ้งเตือน', 'กรุณากรอกเลขไมล์'); return; }
  ...
  const doUpdate = async (status_id: string) => {
    try {
      setUpdating(true);
      const baseParams = {
        request_id: item.request_id, status_id, detail, box,
        user_status: user.status, mile, driver: user.id,
      };
      ...
      if (!netState.isConnected) {
        await addToQueue(
          needsPhoto && photo ? API_ENDPOINTS.UPDATE_PICTURE : API_ENDPOINTS.UPDATE_STATUS,
          needsPhoto && photo ? {...baseParams, picture: photo.base64 ?? ''} : baseParams,
        );
      ...
      if (needsSignatureRating && signature) {
        ...
        await submitSignature({request_id: item.request_id, status_id, picture: cleanSig});
      }
      if (needsSignatureRating) {
        await submitEvaluation({request_id: item.request_id, status_id, eval: String(rating)});
      }
  ...
  return (
    <>
      ...
      <Modal transparent visible={showConfirm} animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.box}>
            <View style={[modalStyles.iconCircle, {backgroundColor: getConfirmColor()}]}>
      ...
      <Modal transparent visible={showSuccess} animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.box}>
            <View style={[modalStyles.iconCircle, {backgroundColor: isOfflineSuccess ? '#e67e22' : (companyColor ?? '#a7cc43')}]}>
        ...
        <View style={styles.card}>
          ...
          {needsSignatureRating && (
            <View style={styles.specialSection}>
              ...
              <View style={styles.signatureActions}>
                <TouchableOpacity style={styles.signatureBtnClear} onPress={handleClearSignature}>
                ...
                >
                  <Text style={[styles.saveText, {color: companyColor}]}>บันทึกลายเซ็น</Text>
                </TouchableOpacity>
              </View>
              {signature
          ...
          {needsPhoto && (
            <View style={styles.photoSection}>
              ...
              {!photo && <Text style={styles.photoHint}>* สถานะนี้ต้องถ่ายรูปก่อนยืนยัน</Text>}
            </View>
          )}
          ...
          {needsBox && (
            <View style={styles.inlineRow}>
              ...
              <View style={styles.inlineInputWrap}>
                ...
                />
                {!box.trim() && <Text style={styles.errorText}>⚠ กรุณากรอกจำนวนกล่อง</Text>}
              </View>
            </View>
          )}
          {/* ── เลขไมล์ ── */}
            ...
            <View style={styles.inlineRow}>
              ...
              <View style={styles.inlineInputWrap}>
                <TextInput
                  ...
                  placeholderTextColor="#aaa"
                />
                {!mile.trim() && <Text style={styles.errorText}>⚠ กรุณากรอกเลขไมล์</Text>}
              </View>
            </View>
          )}
...
inlineInput: {
  ...
  rowLabel:     {fontFamily: 'Quicksand-Bold', fontSize: 12, color: '#373737', width: 110},
  rowSeparator: {fontSize: 12, color: '#373737'},
  rowValue:     {fontSize: 12, color: '#373737', flex: 1, fontFamily: 'Quicksand-Bold'},
  ...
  photoPreview: {
    ...
    borderRadius: 8, marginTop: 10, resizeMode: 'cover',
  },
  ...
  signatureBtnClear: {
    ...
    borderColor: '#e74c3c', alignItems: 'center',
  },
  signatureBtnSave: {
    flex: 1, paddingVertical: 8,
    borderRadius: 6, borderWidth: 1,
