...
export default function ScanScreen({navigation}: Props) {
    ...
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity style={styles.permBtn} onPress={requestPermission}>
          <Text style={[styles.permBtnText, {fontSize: Math.round(width * 0.04)}]}>
            ขอสิทธิ์อีกครั้ง
          </Text>
        </TouchableOpacity>
      </View>
  ...
  if (!device) {
    return (
      <View style={styles.center}>
        <Icon name="camera-off" size={Math.round(width * 0.12)} color="#fff" />
        <Text style={[styles.text, {fontSize: Math.round(width * 0.04)}]}>ไม่พบกล้อง</Text>
      </View>
    );
  }
    ...
    <>
      ...
      <Modal transparent visible={showNotFound} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={[styles.modalIconCircle, {backgroundColor: '#F5A800'}]}>
            ...
            <TouchableOpacity
              style={[styles.modalFullBtn, {backgroundColor: companyColor ?? '#a7cc43'}]}
      ...
      <View style={StyleSheet.absoluteFill}>
        ...
        <Camera
          ...
          device={device}
          isActive={!showManual && !scanning && !showNotFound}
          torch={flash}
          {...cameraProps}
        />
        ...
        {scanning && (
          <View style={styles.scanningOverlay}>
            <Icon name="line-scan" size={Math.round(width * 0.1)} color="#fff" />
            <Text style={[styles.scanningText, {fontSize: Math.round(width * 0.04)}]}>
              กำลังอ่านบาร์โค้ด...
            </Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  manualInput: {
    ...
    padding: 14, color: '#333', marginBottom: 16, letterSpacing: 1, marginTop: 16,
  },
  ...
  modalBox: {
    ...
    padding: 24, width: '80%', alignItems: 'center', elevation: 5,
  },
  modalIconCircle: {
